//
//  BreatheEffectViewController.swift
//  McccSymbols_Example
//
...
class BreatheEffectViewController: UIViewController {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        // 1. 整体呼吸（plain），循环
        imageView1.addSymbolEffect(.breathe.wholeSymbol.plain, options: .repeating)
        
        // 2. 脉冲呼吸（pulse），循环，慢速
        imageView2.addSymbolEffect(.breathe.pulse, options: .repeating.speed(0.4))
        
        // 3. 分层呼吸（pulse），循环
        imageView3.addSymbolEffect(.breathe.byLayer.pulse, options: .repeating)
    }
    ...
    // MARK: - Labels
    lazy var label1: UILabel = makeLabel(text: "整体呼吸 Plain / 循环")
    lazy var label2: UILabel = makeLabel(text: "脉冲呼吸 Pulse / 慢速循环")
    lazy var label3: UILabel = makeLabel(text: "分层脉冲呼吸 / 循环")
